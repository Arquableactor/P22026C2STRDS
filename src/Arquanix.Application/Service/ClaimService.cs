...
public class ClaimService : IClaimService
{
    ...
    public async Task<ServiceResult<List<ClaimDto>>> GetAllAsync()
    {
        var claims = await _repository.GetAllAsync();
        return ServiceResult<List<ClaimDto>>.Ok(claims.Select(ToDto).ToList());
    }
    ...
    public async Task<ServiceResult<ClaimDto>> GetByIdAsync(int id)
    {
        ...
        return claim is null
            ? ServiceResult<ClaimDto>.NotFound()
            : ServiceResult<ClaimDto>.Ok(ToDto(claim));
    }
